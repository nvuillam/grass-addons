...
if [ $num_points -ne $num_points_expected ]; then
	echo >&2 "number of points ($num_points) != requested points per category times number of categories ($num_points_expected)"
	exit 1
fi
...
if [ $num_cats -ne $num_cats_expected ]; then
	echo >&2 "number of sampled categories ($num_cats) != number of categories in raster ($num_cats_expected)"
	exit 1
fi
