...
(
	cd $TARGETHTMLDIR
	rm -rf barscales colortables icons northarrows
)
...
(
	cd $TARGETHTMLDIR/
	sed -i -e "s+</table>+</table><\!\-\- injected in cron_grass8_relbranch_build_binaries.sh \-\-> <center><iframe src=\"https://duckduckgo.com/search.html?site=grass.osgeo.org\&prefill=Search manual pages at DuckDuckGo\" style=\"overflow:hidden;margin:0;padding:0;width:410px;height:40px;\" frameborder=\"0\"></iframe></center>+g" index.html
)
...
$MYMAKE htmldocs-single || (
	echo "$0 htmldocs-single: an error occured"
	exit 1
)
...
(
	cd $TARGETHTMLDIR/..
	rm -f $TARGETHTMLDIR/*html_manual.zip
	zip -r /tmp/grass-${DOTVERSION}_html_manual.zip manuals/
)
...
(
	cd ~/src/grass$GMAJOR-addons/
	git checkout grass$GMAJOR
	git pull origin grass$GMAJOR
)
# compile addons
cd $GRASSBUILDDIR
